  ...
  <div class="wrap height-full"
    ...
    <div class="box">
      ...
      <div class="con-padding-horizontal search-wrap">
        ...
        <router-link :to="{name:'SeeProcessMyApply'}">返回</router-link>
      </x-button>
        </div>
      </div>
    </div>
    ...
    <div class="content-wrap">
      <ul class="content-header">
       ...
       <li class="list name c-pointer" @click="changeOrder('ActivityState')" >
          节点状态
          <i :class="[{ 'el-icon-d-caret': headOrder.ActivityState === 0 }, { 'el-icon-caret-top': headOrder.ActivityState === 1 }, { 'el-icon-caret-bottom': headOrder.ActivityState === 2 }]"></i>
       </li>
               <li class="list name c-pointer" @click="changeOrder('lastUpdatedByUserName')" >
          经办人
          <i :class="[{ 'el-icon-d-caret': headOrder.lastUpdatedByUserName === 0 }, { 'el-icon-caret-top': headOrder.lastUpdatedByUserName === 1 }, { 'el-icon-caret-bottom': headOrder.lastUpdatedByUserName === 2 }]"></i>
       </li>
    ...
    searchResult (page) {
      ...
      api.getProcessHisPage(parm).then(res => {
        ...
        res.data.rows.map(item => {
          ...
          } else if (item.activityState === 4) {
            item.activityState = 'Completed'
          } else if (item.activityState === 5) {
            item.activityState = 'Suspended'
          } else if (item.activityState === 6) {
            item.activityState = 'Withdrawed'
          } else if (item.activityState === 7) {
            item.activityState = 'Sendbacked'
          } else if (item.activityState === 8) {
            item.activityState = 'Cancelled'
          } else if (item.activityState === 9) {
            item.activityState = 'Aborted'
          }
        })
