  ...
  <div class="wrap height-full"
    ...
    <div class="content-wrap">
      ...
      <div class="scroll">
        <el-scrollbar>
          <ul class="list-wrap">
            <li class="list" v-for="item in ePlanList" :key="item.key">
              <div class="list-content">
                <div class="checkbox">
                  <input type="checkbox" v-model="ePlanID" :value="item.id" @change="emitEditID">
                </div>
                <div class="number">{{ item.id }}</div>
                <div class="name word-break">{{ item.scene }}</div>
                <div class="url word-break">{{ item.keyword }}</div>
                ...
                <div class="url word-break">{{ item.deptName }}</div>
...
<script>
import { transformDate, PDF_UPLOADED_VIEW_URL, FILE_SERVER_PATH } from '@/common/js/utils.js'
...
export default {
  ...
  methods: {
    ...
    // 弹框确认是否删除
    dialogEnter () {
      api.delEPlan(this.ePlanID.join(',')).then(res => {
        if (res.code === 0) {
          this.ePlanID = []
          this.$message({
            message: '删除成功',
    ...
    // 全选
    checkAll () {
      this.bCheckAll ? this.ePlanList.map(val => this.ePlanID.push(val.id)) : this.ePlanID = []
      this.emitEditID()
    },
