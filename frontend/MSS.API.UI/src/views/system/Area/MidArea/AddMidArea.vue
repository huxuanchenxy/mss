...
export default {
  ...
  methods: {
    ...
    enter () {
      ...
      let path1=this.$route.params.mark
    let TB_Config_BigArea ={  
      ...
      if (this.isShow === 'add') {
        // 添加站区  
        api.SaveConfigBigArea(TB_Config_BigArea).then(res => {
          if (res.ret === 0) {
            this.$message({
              message: '添加成功',
              type: 'success',
        ...
        TB_Config_BigArea.id = this.AreaID
        // 修改权限组
         api.UpdateConfigBigAre(TB_Config_BigArea).then(res => {
          if (res.code === 0) {
            this.$message({
              message: '修改成功',
              type: 'success',
